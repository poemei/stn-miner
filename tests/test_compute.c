...
)
{
    ...
    inventory->providers[0].buffer_ready = 1;
    ...
    inventory->providers[1].buffer_ready = 0;
...
int main(void)
{
    ...
    check(
        inventory.providers[0].buffer_ready == 1,
        "OpenCL buffer readiness preserved"
    );

    check(
        inventory.providers[0].transfer_ready == 1,
        "OpenCL transfer readiness preserved"
    );

    check(
        inventory.providers[0].program_ready == 1,
        "OpenCL program object readiness preserved"
    );
    ...
    check(
        inventory.providers[0].argument_ready == 1,
        "OpenCL kernel argument readiness preserved"
    );
