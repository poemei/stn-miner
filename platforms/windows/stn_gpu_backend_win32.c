...
typedef int (__stdcall *stn_cl_get_device_ids_fn)(
    void *,
    unsigned long long,
    unsigned int,
    void **,
...
typedef void *(__stdcall *stn_cl_create_context_fn)(
    const intptr_t *,
    unsigned int,
    void * const *,
    void *,
...
typedef int (__stdcall *stn_cl_release_context_fn)(
    void *
);

typedef void *(__stdcall *stn_cl_create_queue_fn)(
    void *,
    void *,
    unsigned long long,
    int *
...
typedef int (__stdcall *stn_cl_release_queue_fn)(
    void *
);

typedef void *(__stdcall *stn_cl_create_program_fn)(
    void *,
    unsigned int,
    const char **,
    const size_t *,
...
typedef int (__stdcall *stn_cl_build_program_fn)(
    void *,
    unsigned int,
    void * const *,
    const char *,
...
typedef int (__stdcall *stn_cl_release_program_fn)(
    void *
);

typedef void *(__stdcall *stn_cl_create_kernel_fn)(
    void *,
    const char *,
    int *
);

typedef int (__stdcall *stn_cl_release_kernel_fn)(
    void *
);

typedef void *(__stdcall *stn_cl_create_buffer_fn)(
    void *,
    unsigned long long,
    size_t,
    void *,
...
typedef int (__stdcall *stn_cl_release_mem_fn)(
    void *
);

typedef int (__stdcall *stn_cl_set_kernel_arg_fn)(
    void *,
    unsigned int,
    size_t,
    const void *
...
typedef int (__stdcall *stn_cl_enqueue_write_fn)(
    void *,
    void *,
    unsigned int,
    size_t,
...
typedef int (__stdcall *stn_cl_enqueue_read_fn)(
    void *,
    void *,
    unsigned int,
    size_t,
...
typedef int (__stdcall *stn_cl_enqueue_kernel_fn)(
    void *,
    void *,
    unsigned int,
    const size_t *,
...
typedef int (__stdcall *stn_cl_finish_fn)(
    void *
);
...
)
{
    ...
    state.get_platform_ids =
        ...
        GetProcAddress(state.module, "clGetPlatformIDs");

    state.get_device_ids =
        (stn_cl_get_device_ids_fn)
        GetProcAddress(state.module, "clGetDeviceIDs");

    state.create_context =
        (stn_cl_create_context_fn)
        GetProcAddress(state.module, "clCreateContext");

    state.release_context =
        (stn_cl_release_context_fn)
        GetProcAddress(state.module, "clReleaseContext");

    state.create_queue =
        (stn_cl_create_queue_fn)
        GetProcAddress(state.module, "clCreateCommandQueue");

    state.release_queue =
        (stn_cl_release_queue_fn)
        GetProcAddress(state.module, "clReleaseCommandQueue");

    state.create_program =
        (stn_cl_create_program_fn)
        GetProcAddress(state.module, "clCreateProgramWithSource");

    state.build_program =
        (stn_cl_build_program_fn)
        GetProcAddress(state.module, "clBuildProgram");

    state.release_program =
        (stn_cl_release_program_fn)
        GetProcAddress(state.module, "clReleaseProgram");

    state.create_kernel =
        (stn_cl_create_kernel_fn)
        GetProcAddress(state.module, "clCreateKernel");

    state.release_kernel =
        (stn_cl_release_kernel_fn)
        GetProcAddress(state.module, "clReleaseKernel");

    state.create_buffer =
        (stn_cl_create_buffer_fn)
        GetProcAddress(state.module, "clCreateBuffer");

    state.release_mem =
        (stn_cl_release_mem_fn)
        GetProcAddress(state.module, "clReleaseMemObject");

    state.set_kernel_arg =
        (stn_cl_set_kernel_arg_fn)
        GetProcAddress(state.module, "clSetKernelArg");

    state.enqueue_write =
        (stn_cl_enqueue_write_fn)
        GetProcAddress(state.module, "clEnqueueWriteBuffer");

    state.enqueue_read =
        (stn_cl_enqueue_read_fn)
        GetProcAddress(state.module, "clEnqueueReadBuffer");

    state.enqueue_kernel =
        (stn_cl_enqueue_kernel_fn)
        GetProcAddress(state.module, "clEnqueueNDRangeKernel");

    state.finish =
        (stn_cl_finish_fn)
        GetProcAddress(state.module, "clFinish");
